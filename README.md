...
2. **AI Review Generator**: On product pages, use the AI generator to create reviews (requires an API key for your chosen provider in Settings). On Amazon's review page, successful generation fills the title, body, and selected star rating—including the current in-context star widget—then collapses the panel to a "🤖 AI Review Generator" button
